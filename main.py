...
def iterate(key):
    ...
    for recipe_url in recipe_urls:
        ...
        for ingr in ingr_elements:
            ...
            # Check if any keywords match for this recipe
            pattern = r'\b(?:' + '|'.join(re.escape(keyword) for k in keyword) + r')\b'
            if any(re.search(pattern, line, re.IGNORECASE) for line in ingr_text.strip().split('\n')):
                matching_recipes.append(recipe_url)
                matching_ingredients.append(ingr_text)  # Store matching ingredients`
